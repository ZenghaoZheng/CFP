...
class Human36mDataset(MocapDataset):
    def __init__(self, path, opt, remove_static_joints=True):
        ...
        for cameras in self._cameras.values():  # cameras是一个列表
            for i, cam in enumerate(cameras):   # cam是cameras列表中的每一个字典，i代表的是第几个字典
                cam.update(h36m_cameras_intrinsic_params[i])    # 更新cam字典，将h36m_cameras_intrinsic_params中第i个字典的内容添加到cam后
                for k, v in cam.items():    # k, v 是更新后的cam键值
                    if k not in ['id', 'res_w', 'res_h']:
                        cam[k] = np.array(v, dtype='float32')  # 如果当前k不是id,res_w,res_h，将v转变成array的形式

                if opt.crop_uv == 0:    # crop_uv是什么意思？
                    cam['center'] = normalize_screen_coordinates(cam['center'], w=cam['res_w'], h=cam['res_h']).astype(
                        'float32')  # 一个不知道什么的操作把center变化了
                    cam['focal_length'] = cam['focal_length'] / cam['res_w'] * 2    # 把focal_length也变了

                if 'translation' in cam:
                    cam['translation'] = cam['translation'] / 1000 

                cam['intrinsic'] = np.concatenate((cam['focal_length'],     # 这里新加入intrinsic键对应的值为concat后面的内容
                                                   cam['center'],
                                                   cam['radial_distortion'],
                                                   cam['tangential_distortion']))

        data = np.load(path,allow_pickle=True)['positions_3d'].item()   #读取h36m_npz文件
        ...
        for subject, actions in data.items():
            ...
            for action_name, positions in actions.items():
                self._data[subject][action_name] = {
                    'positions': positions,
                    'cameras': self._cameras[subject],  # _cameras已经在前面处理过了
                }
