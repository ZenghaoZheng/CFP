...
from model.module_cfp.dct import dct,idct
from einops import rearrange
import torch.nn.functional as F
class DCTAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., dct_percentage=0.6):
        super().__init__()
    ...
    def forward(self, x):
        B, N, C = x.shape
        x = dct(x.permute(0, 2, 1))[:, :, :math.ceil(N * self.dct_percentage)].permute(0, 2, 1).contiguous()
        x = idct(x.permute(0, 2, 1)).permute(0, 2, 1).contiguous()
        short_x = x.clone()
        B, N, C = x.shape
...
class SHR_Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_hidden_dim, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, dct_ratio=0.6):
        super().__init__()
        self.norm1_1 = norm_layer(dim)
        self.norm1_2 = norm_layer(dim)
        self.norm1_3 = norm_layer(dim)

        self.attn_1 = DCTAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, \
            qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, dct_percentage=dct_ratio)
        self.attn_2 = DCTAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, \
            qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, dct_percentage=dct_ratio)
        self.attn_3 = DCTAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, \
            qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, dct_percentage=dct_ratio)
    ...
    def forward(self, x_1, x_2, x_3):
        x_1, short_x1 = self.attn_1(self.norm1_1(x_1))
        x_1 = short_x1 + self.drop_path(x_1)
        x_2, short_x2 = self.attn_2(self.norm1_2(x_2))
        x_2 = short_x2 + self.drop_path(x_2)
        x_3, short_x3 = self.attn_3(self.norm1_3(x_3))
        x_3 = short_x3 + self.drop_path(x_3)
...
class Transformer(nn.Module):
    def __init__(self, depth=3, embed_dim=512, mlp_hidden_dim=1024, h=8, drop_rate=0.1, length=27, dct_ratio=0.6, freq_prune_layer=[]):
        super().__init__()
        drop_path_rate = 0.20
        attn_drop_rate = 0.
        qkv_bias = True
        ...
        self.SHR_blocks = nn.ModuleList([
            SHR_Block(
                ...
                dct_ratio=self.dct_ratio if i in self.freq_prune_layer else 1)
    ...
    def forward(self, x_1, x_2, x_3):
        ...
        global_residual = {'x_1':[], 'x_2':[], 'x_3':[]}
        for i, blk in enumerate(self.SHR_blocks):
            x_1, x_2, x_3 = self.SHR_blocks[i](x_1, x_2, x_3)
            global_residual['x_1'].append(x_1)
            global_residual['x_2'].append(x_2)
            global_residual['x_3'].append(x_2)

        residual = global_residual['x_1']
        x_1 = 0
        for global_x in residual:
            global_x = rearrange(global_x, 'b f c -> b c f')
            global_x = F.interpolate(global_x, size=(f), mode='nearest')
            global_x = rearrange(global_x, 'b c f -> b f c')
            x_1 = x_1 + global_x
        residual = global_residual['x_2']
        x_2 = 0
        for global_x in residual:
            global_x = rearrange(global_x, 'b f c -> b c f')
            global_x = F.interpolate(global_x, size=(f), mode='nearest')
