...
from model.mixste_dct_3dhp import Model
from model.block.stmo_pretrain import Model_MAE
...
def step(split, opt, actions, dataLoader, model, optimizer=None, epoch=None):
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if opt.MAE:
            ...
            [input_2D, batch_cam, scale, bb_box] = get_variable(split, [input_2D, batch_cam, scale, bb_box])
            ...
            [input_2D, gt_3D, batch_cam, scale, bb_box] = get_variable(split,
        ...
        if split == 'train':
            ...
            if opt.MAE:
                ...
                #                                     subject,MAE=opt.MAE)
                joint_error_test = mpjpe_cal(torch.cat((input_2D[:, ~mask], input_2D[:, mask]), dim=1),
                                             output_2D).item()
            else:
                pred_out[:, :, 14, :] = 0
...
if __name__ == '__main__':
    ...
    for epoch in range(1, opt.nepoch):
        ...
        if opt.test_in_train == 1:
            ...
            if opt.train and data_threshold < opt.previous_best:
                if opt.MAE:
                ...
                opt.previous_best = data_threshold
            ...
            if opt.train == 0:
                ...
                if opt.MAE:
                    ...
                    logging.info(
                        'epoch: %d, lr: %.7f, loss: %.4f, MPJPE: %.2f' % (epoch, lr, loss, p1))
                    print('e: %d, lr: %.7f, loss: %.4f, M: %.2f' % (epoch, lr, loss, p1))
