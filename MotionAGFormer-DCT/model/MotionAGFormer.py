...
from model.modules.xboost import xboost
...
class AGFormerBlock(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., act_layer=nn.GELU, attn_drop=0., drop=0., drop_path=0.,
                 ...
                 temporal_connection_len=1, neighbour_num=4, n_frames=243, dct_ratio=0.6):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        if mixer_type == 'attention':
            self.mixer = Attention(dim, dim, num_heads, qkv_bias, qk_scale, attn_drop,
                                   proj_drop=drop, mode=mode, dct_percentage=dct_ratio)
            ...
            self.mixer = GCN(dim, dim,
                             ...
                             temporal_connection_len=temporal_connection_len, dct_percentage=dct_ratio)
        elif mixer_type == "ms-tcn":
            self.mixer = MultiScaleTCN(in_channels=dim, out_channels=dim, dct_percentage=dct_ratio)
        else:
    ...
    def forward(self, x):
        ...
        if self.use_layer_scale:
            x, shortx = self.mixer(self.norm1(x))
            x = self.drop_path(self.layer_scale_1.unsqueeze(0).unsqueeze(0) * x)
            x = x + shortx
            x = x + self.drop_path(
...
class MotionAGFormerBlock(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., act_layer=nn.GELU, attn_drop=0., drop=0., drop_path=0.,
                 ...
                 temporal_connection_len=1, use_tcn=False, graph_only=False, neighbour_num=4, n_frames=243, dct_ratio=0.6):
        super().__init__()
        ...
        self.att_spatial = AGFormerBlock(dim, mlp_ratio, act_layer, attn_drop, drop, drop_path, num_heads, qkv_bias,
                                         ...
                                         use_temporal_similarity=use_temporal_similarity,
                                         neighbour_num=neighbour_num,
                                         n_frames=n_frames, dct_ratio=dct_ratio)
        ...
        if graph_only:
            self.graph_spatial = GCN(dim, dim,
                                     num_nodes=17,
                                     mode='spatial', dct_percentage=dct_ratio)
            if use_tcn:
                self.graph_temporal = MultiScaleTCN(in_channels=dim, out_channels=dim, dct_percentage=dct_ratio)
            else:
...
def create_layers(dim, n_layers, mlp_ratio=4., act_layer=nn.GELU, attn_drop=0., drop_rate=0., drop_path_rate=0.,
                  ...
                  dct_ratio=0.5, freq_prune_layer = []):
    """
    ...
    for i in range(n_layers):
        if i in freq_prune_layer:
            n_frames = math.ceil(n_frames*dct_ratio)
            c += 1
    ...
    for i in range(n_layers):
        layers.append(MotionAGFormerBlock(dim=dim,
                                          ...
                                          neighbour_num=neighbour_num,
                                          n_frames=frames_list[i],
                                          dct_ratio= dct_ratio if i in freq_prune_layer else 1))
...
class MotionAGFormer(nn.Module):
    ...
    def __init__(self, n_layers, dim_in, dim_feat, dim_rep=512, dim_out=3, mlp_ratio=4, act_layer=nn.GELU, attn_drop=0.,
                 ...
                 neighbour_num=4, n_frames=243, dct_ratio=0.6, freq_prune_layer=[]):
        """
        ...
        self.layers = create_layers(dim=dim_feat,
                                    ...
                                    freq_prune_layer=self.freq_prune_layer)
...
def _test():
    ...
    b, t, j, c = 1, 81, 17, 3
    random_x = torch.randn((b, t, j, c))

    model = MotionAGFormer(n_layers=16, dim_in=3, dim_feat=128, dim_rep=512, dim_out=3, mlp_ratio=4, act_layer=nn.GELU, attn_drop=0.,
                 drop=0., drop_path=0., use_layer_scale=True, layer_scale_init_value=1e-5, use_adaptive_fusion=True,
                 num_heads=8, qkv_bias=False, qkv_scale=None, hierarchical=False, num_joints=17,
                 use_temporal_similarity=True, temporal_connection_len=1, use_tcn=False, graph_only=False,
                 neighbour_num=2, n_frames=81, dct_ratio=0.7, freq_prune_layer=[3,7,11,15])
    model.eval()
