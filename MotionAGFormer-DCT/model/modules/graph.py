...
class GCN(nn.Module):
    def __init__(self, dim_in, dim_out, num_nodes, neighbour_num=4, mode='spatial', use_temporal_similarity=True,
                 temporal_connection_len=1, connections=None, dct_percentage=0.6):
        self.nodes_ = """
...
if __name__ == "__main__":
    gcn = GCN(dim_in=528, dim_out=528, num_nodes=17, mode='spatial')
    x = torch.randn(8, 243, 17, 528)
    gcn.forward(x)
    for name, param in gcn.named_parameters():
        print(f'{name}: {param.numel()}')
    print(sum(p.numel() for p in gcn.parameters() if p.requires_grad))