...
def get_pose3D(video_path, output_dir):
    ...
    ## Reload 
    model = nn.DataParallel(DSTformer(dim_in=3, dim_out=3, dim_feat=512, dim_rep=512,
    ...
    model_path = sorted(glob.glob(os.path.join('checkpoint/pretrained', 'best_epoch.bin')))[0]
