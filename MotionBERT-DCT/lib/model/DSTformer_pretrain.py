...
class Block(nn.Module):
    ...
    def forward(self, x, seqlen=1):
        if self.st_mode=='stage_st':
            ...
            x = dct(x.permute(0, 2, 3, 1))[..., :math.ceil(seqlen * self.dct_ratio)].permute(0, 3, 1, 2).contiguous()
            x = idct(x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2).contiguous()
            seqlen = x.shape[1]
            x = x.reshape(-1, N, C)
            shortx = x.clone()
            x = self.attn_t(self.norm1_t(shortx), seqlen=seqlen)
            ...
            x = x.reshape(-1, N, C)
            shortx = x.clone()
            x = self.attn_t(self.norm1_t(x), seqlen=seqlen)
            x = shortx + self.drop_path(x)
            x = x + self.drop_path(self.mlp_t(self.norm2_t(x)))
...
class DSTformer_pretrain(nn.Module):
    def __init__(self, dim_in=3, dim_out=3, dim_feat=256, dim_rep=512,
                 ...
                 dct_ratio=0.6, freq_prune_layer=[]):
        super().__init__()
        self.dct_ratio = dct_ratio
        ...
        self.blocks_st = nn.ModuleList([
            Block(
                ...
                st_mode="stage_st", dct_ratio=self.dct_ratio if i in self.freq_prune_layer else 1)
            for i in range(depth)])
        self.blocks_ts = nn.ModuleList([
            Block(
                dim=dim_feat, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, 
                st_mode="stage_ts", dct_ratio=self.dct_ratio if i in self.freq_prune_layer else 1)
            for i in range(depth)])
    ...
    def forward(self, x, return_rep=False):   
        ...
        for idx, (blk_st, blk_ts) in enumerate(zip(self.blocks_st, self.blocks_ts)):
            ...
            if self.att_fuse:
                ...
                x = (x_st + x_ts)*0.5
            global_residual.append(x.reshape(B,-1,J,C))
