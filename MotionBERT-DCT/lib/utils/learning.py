...
from lib.model.DSTformer_pretrain import DSTformer_pretrain
...
def load_backbone(args):
    ...
    elif args.backbone=='DSTformer_pretrain':
        model_backbone = DSTformer_pretrain(dim_in=3, dim_out=3, dim_feat=args.dim_feat, dim_rep=args.dim_rep,
                                   depth=args.depth, num_heads=args.num_heads, mlp_ratio=args.mlp_ratio,
                                   norm_layer=partial(nn.LayerNorm, eps=1e-6),
                                   maxlen=args.maxlen, num_joints=args.num_joints, dct_ratio=args.dct_ratio, freq_prune_layer=args.freq_prune_layer)
    elif args.backbone=='DSTformer_main':
        model_backbone = DSTformer_main(dim_in=3, dim_out=3, dim_feat=args.dim_feat, dim_rep=args.dim_rep,
                                   depth=args.depth, num_heads=args.num_heads, mlp_ratio=args.mlp_ratio,
                                   norm_layer=partial(nn.LayerNorm, eps=1e-6),
