...
def train_epoch(args, model_pos, train_loader, losses, optimizer, has_3d, has_gt):
    ...
    for idx, (batch_input, batch_gt) in tqdm(enumerate(train_loader)):    
        ...
        optimizer.zero_grad()
        if has_3d:
            gt_freq = dct(batch_gt.permute(0, 2, 3, 1)).permute(0, 3, 1, 2).contiguous()
            predicted_freq = dct(predicted_3d_pos.permute(0, 2, 3, 1)).permute(0, 3, 1, 2).contiguous()
            loss_freq = loss_mpjpe(predicted_freq, gt_freq)
...
def train_with_config(args, opts):
    ...
    if args.finetune:
        if opts.resume or opts.evaluate:
            ...
            # model_backbone.load_state_dict(pretrained_dict)
    ...
    if not opts.evaluate:        
        ...
        for epoch in range(st, args.epochs):
            ...
            if e1 < min_loss:
                min_loss = e1
                print('Saving checkpoint to', chk_path_best)
